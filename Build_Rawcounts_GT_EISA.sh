...
grep '>' ${t}/c_elegans.PRJNA13758.WS279.mRNA_transcripts.fa | sed 's/>//g' | sed 's/gene=//g' > gene_tx_list.txt
grep '>' ${g}/CE_genes_seq.fa | sed 's/>//g' > gene_gen_list.txt
...
for s in ${sample[@]}; do
	for r in ${rep[@]}; do
		...
		echo ${s}${r} > NumReads_gen_${s}${r}.sf && awk 'NR==FNR{a[$1];next} $1 in a{print $5}' gene_gen_list.txt ${g}/${d}/${s}${r}/quant.sf >> NumReads_gen_${s}${r}.sf
		echo ${s}${r} > NumReads_tx_${s}${r}.sf && awk 'NR==FNR{a[$1];next} $1 in a{print $5}' gene_tx_list.txt ${t}/${d}/${s}${r}/quant.sf >> NumReads_tx_${s}${r}.sf
	done
done
...
echo "Gene_IDs" > order_WBGenes_GE && awk 'NR==FNR{a[$1];next} $1 in a{print $2}' ${g}/${d}/${s[0]}${r[0]}/quant.sf gene_gen_list.txt >> order_WBGenes_GE
echo "Gene_IDs" > order_WBGenes_Tx && awk 'NR==FNR{a[$1];next} $1 in a{print $2}' ${t}/${d}/${s[0]}${r[0]}/quant.sf gene_tx_list.txt >> order_WBGenes_Tx
